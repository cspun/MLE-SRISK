...
#    weekret_future = weekret.shift(-h)
    ...
    xydata_train =xydata
    xdata = xydata_train[['Rsse_t','X3T_change', 'change_slope', 'ted', 'cre_spread',
    ...
    xdata_predict = xydata[['Rsse_t','X3T_change', 'change_slope', 'ted', 'cre_spread',
...
firmdata_raw = firmdata_raw[['GVKEY','COMNAM','RET','VOL','weekdaynum','Rjh']]
...
market_raw=pd.read_csv('3step Hong Kong Market Data with topVaR ANN1.csv')
market_raw.index = pd.to_datetime(market_raw.date)
market_raw=market_raw.drop('date',1)
...
#    weekret_future = weekret.shift(-h)
    xydata=macro_hist_t
    #xydata = pd.concat([macro_hist_t.iloc[:,-7:],weekret_future],axis=1).dropna()
    xydata=xydata.dropna(subset=filter(lambda x: x not in ['VaR_sse','VaR_hsi',
                                                           'VaR_sti'],xydata.columns))
    
    xydata_train =xydata
    xdata = xydata_train[['Rsse_t','Rsseh','Rhsi_t','HK_equ_vol', 'HIBOR',  
           'HIBOR_2','HK_equ_vol_2']].iloc[:-5,:]
    std_scale_x = preprocessing.StandardScaler().fit(xdata)
    xdata = std_scale_x.transform(xdata)
    ...
    ydata = std_scale_y.transform(ydata)
    # xdata in the last dates of this period:xdata[t]
    xdata_predict = xydata[['Rsse_t','Rsseh','Rhsi_t','HK_equ_vol', 'HIBOR',  
           'HIBOR_2','HK_equ_vol_2','VaR_sse']].iloc[xydata.shape[0]-1:xydata.shape[0],:]
    #xdata_predict=xdata_predict.drop(labels='Varh', axis=1)
    
    if not math.isnan(xdata_predict.VaR_sse):
        xdata_predict.Rsseh=xdata_predict.VaR_sse
    
    #drop column Varh
    xdata_predict=xdata_predict.drop(labels=['VaR_sse'], axis=1)
...
#market_raw3.to_csv('3step Hong Kong Market Data with VaRs RandomForest.csv')
...
def cal_MSE_firm_onedate_ann1(start_date_dt,firm_macro_2,firstdate):
    ...
    beforeYears=3
    n_steps = 5 # the length of X data 
    n_inputs_1 = 12   # the number of variables
    n_inputs_2 = 7
    num_layers_0=30
    keep_prob=0.5
    #n_neurons = 10
...
#    global numpy_array_bias_2_2
    ...
    xydata=xydata.dropna(subset=filter(lambda x: x not in ['VaR_sp500','VaR_sse','VaR_hsi',
                                                           'VaR_sti','MES_hsi'],xydata.columns))
...
#           're_excess_2','equ_vol_2','Rsysh']].iloc[:-5,:]
    xdata = xydata_train[['VOL', 'RET', 'HK_equ_vol', 'HIBOR', 'VOL_2',
           'HIBOR_2','HK_equ_vol_2','Rhsih','Rhsi_t','Rsseh','Rsse_t']].iloc[:-5,:]
    std_scale_x = preprocessing.StandardScaler().fit(xdata)
    xdata = std_scale_x.transform(xdata)
...
#           're_excess_2','equ_vol_2','Rsysh','Varh']].iloc[xydata.shape[0]-1:xydata.shape[0],:]
    xdata_predict = xydata[['VOL', 'RET', 'HK_equ_vol', 'HIBOR', 'VOL_2',
           'HIBOR_2','HK_equ_vol_2','Rhsih','Rhsi_t','Rsseh','Rsse_t',
           'MES_hsi','VaR_sse']].iloc[xydata.shape[0]-1:xydata.shape[0],:]
    ...
    #drop column Varh
    xdata_predict=xdata_predict.drop(labels=['MES_hsi','VaR_sse'], axis=1)
...
#    dates = list(date_df.index) 
    ...
    for i in range(len(dates)):
        ...
        firm_MES.to_csv(os.path.join(path,r'HK_3Step_Adaboost_singlefirm_%d.csv'%firmcode))
        gc.collect()
...
for firmcode in firmcodes:
    try:
        mes = cal_MSE_firm(firmcode)
        mes.to_csv(os.path.join(path,r'HK_3Step_Adaboost_singlefirm_%d.csv'%firmcode))
        all_mes.append(mes)
        firm_MES_all = pd.concat(all_mes)
        firm_MES_all.to_csv('HK_3Step_Adaboost.csv')
    except:
        pass
