...
os.chdir(r'C:\Users\Administrator\Google 云端硬盘\SRISKwork\Comparison\US Two-step regression')
...
firmdata_raw = pd.read_csv('US Company Features with 4 additional features.csv')
...
for fvariable in list(firmdata_raw.columns.values):
        new_var=[]
        for j in firmdata_raw[fvariable]:
            ...
            try:
                ...
                pass
            new_var.append(a)
        firmdata_raw.loc[:,fvariable] = new_var
...
#           're_excess_2','equ_vol_2','Rsysh']].iloc[:-5,:]
    xdata = xydata_train[['VOL', 'RET', 'X3T_change', 'change_slope', 'ted', 'cre_spread',
           ...
           're_excess_2','equ_vol_2','size','maturity_mismatch','leverage','boom','size_2',
           'maturity_mismatch_2','leverage_2','boom_2',
           'Rsysh']].iloc[:-5,:]
    std_scale_x = preprocessing.StandardScaler().fit(xdata)
    xdata = std_scale_x.transform(xdata)
    ...
    ydata = xydata_train[['Rjh']].iloc[n_steps-1:-n_steps,:]
    std_scale_y = preprocessing.StandardScaler().fit(ydata)
    ydata = std_scale_y.transform(ydata)
...
#           're_excess_2','equ_vol_2','Rsysh','Varh']].iloc[xydata.shape[0]-1:xydata.shape[0],:]
    xdata_predict = xydata[['VOL', 'RET', 'X3T_change', 'change_slope', 'ted', 'cre_spread',
           're_excess','equ_vol', 'VOL_2','RET_2','X3T_change_2','change_slope_2','ted_2','cre_spread_2','sp500',
           're_excess_2','equ_vol_2','size','maturity_mismatch','leverage','boom','size_2',
           'maturity_mismatch_2','leverage_2','boom_2','Rsysh','Varh']].iloc[-n_steps:,:]


    
    if not math.isnan(xdata_predict.Varh[n_steps-1]):
        xdata_predict.Rsysh[n_steps-1]=xdata_predict.Varh[n_steps-1]
    ...
    #random.seed(0)
    np.random.seed(0)
    X = tf.placeholder(tf.float32, [None, n_steps, xdata.shape[2]])
    y = tf.placeholder(tf.float32, [None,1])
    
    # BasicRNNCell
    #cell1 = tf.contrib.rnn.BasicRNNCell(num_units=10)
    #cell2 = tf.contrib.rnn.BasicRNNCell(num_units=10)
    #cell3 = tf.contrib.rnn.BasicRNNCell(num_units=10)
    
    # LSTM
    cell1=tf.nn.rnn_cell.LSTMCell(num_units=10)
    cell2=tf.nn.rnn_cell.LSTMCell(num_units=10)
    cell3=tf.nn.rnn_cell.LSTMCell(num_units=10)
    
    
    multi_layer_cell = tf.contrib.rnn.MultiRNNCell([cell1,cell2,cell3])
    
    #cell = tf.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=0.5)
    
    # states of multiple layer
    outputs, states = tf.nn.dynamic_rnn(multi_layer_cell, X, dtype=tf.float32)
    # states of single layer
    #outputs, states = tf.nn.dynamic_rnn(cell1, X, dtype=tf.float32)
    
    # BasicRNN and GRU multiple and single layer
    #out = fully_connected(states[-1], n_outputs, activation_fn=None)
    # LSTM get short state multiple layer
    out = fully_connected(states[-1][1], n_outputs, activation_fn=None)
...
#    dates = list(date_df.index) 
    ...
    for i in range(len(dates)):
        ...
        firm_MES.to_csv(os.path.join(path,r'US_2Step_RandomForestwith4additionalfeatures_singlefirm_%d.csv'%firmcode))
        gc.collect()
...
for firmcode in firmcodes:
    try:
        mes = cal_MSE_firm(firmcode)
        mes.to_csv(os.path.join(path,r'US_2Step_RandomForestwith4additionalfeatures_singlefirm_%d.csv'%firmcode))
        all_mes.append(mes)
        firm_MES_all = pd.concat(all_mes)
        firm_MES_all.to_csv('US_2Step_RandomForestwith4additionalfeatures.csv')
    except:
        pass
