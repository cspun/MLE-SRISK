...
#           're_excess_2','equ_vol_2','Rsysh','Varh']].iloc[xydata.shape[0]-1:xydata.shape[0],:]
    ...
    # LSTM
    cell1=tf.nn.rnn_cell.LSTMCell(num_units=50)
    cell2=tf.nn.rnn_cell.LSTMCell(num_units=50)
...
#    dates = list(date_df.index) 
    ...
    for i in range(len(dates)):
        ...
        firm_MES.to_csv(os.path.join(path,r'US_2Step_MES_LSTM2_singlefirm_%d.csv'%firmcode))
        gc.collect()
...
for firmcode in firmcodes:
    try:
        mes = cal_MSE_firm(firmcode)
        mes.to_csv(os.path.join(path,r'US_2Step_MES_LSTM2_singlefirm_%d.csv'%firmcode))
        all_mes.append(mes)
        firm_MES_all = pd.concat(all_mes)
        firm_MES_all.to_csv('US_2Step_MES_LSTM2.csv')
    except:
        pass
