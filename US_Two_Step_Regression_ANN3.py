...
#    dates = list(date_df.index) 
    ...
    for i in range(len(dates)):
        ...
        firm_MES.to_csv(os.path.join(path,r'US_2Step_ANN3_singlefirm_%d.csv'%firmcode))
        gc.collect()
...
for firmcode in firmcodes:
    try:
        mes = cal_MSE_firm(firmcode)
        mes.to_csv(os.path.join(path,r'US_2Step_ANN3_singlefirm_%d.csv'%firmcode))
        all_mes.append(mes)
        firm_MES_all = pd.concat(all_mes)
        firm_MES_all.to_csv('US_2Step_ANN3.csv')
    except:
        pass
