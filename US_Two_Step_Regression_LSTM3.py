...
#           're_excess_2','equ_vol_2','Rsysh','Varh']].iloc[xydata.shape[0]-1:xydata.shape[0],:]
    xdata_predict = xydata[['VOL', 'RET', 'X3T_change', 'change_slope', 'ted', 'cre_spread',
           're_excess','equ_vol', 'VOL_2','RET_2','X3T_change_2','change_slope_2','ted_2','cre_spread_2','sp500',
           're_excess_2','equ_vol_2','Rsysh','Varh']].iloc[-n_steps:,:]
...
#    dates = list(date_df.index) 
    ...
    for i in range(len(dates)):
        ...
        path=r'C:\Users\Administrator\Google 云端硬盘\SRISKwork\Comparison\US Two-step regression\MES'
        firm_MES.to_csv(os.path.join(path,r'US_2Step_MES_LSTM3_singlefirm_%d.csv'%firmcode))
        gc.collect()
...
for firmcode in firmcodes:
    try:
        mes = cal_MSE_firm(firmcode)
        mes.to_csv(os.path.join(path,r'US_2Step_MES_LSTM3_singlefirm_%d.csv'%firmcode))
        all_mes.append(mes)
        firm_MES_all = pd.concat(all_mes)
        firm_MES_all.to_csv('US_2Step_MES_LSTM3.csv')
    except:
        pass
