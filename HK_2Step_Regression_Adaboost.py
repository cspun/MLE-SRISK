...
market_raw=pd.read_csv('2step Hong Kong Market Data with topVaR ANN1.csv')
market_raw.index = pd.to_datetime(market_raw.date)
market_raw=market_raw.drop('date',1)
...
#    dates = list(date_df.index) 
    ...
    for i in range(len(dates)):
        ...
        new,new2=cal_MSE_firm_onedate_svr(firm_macro_2=firm_macro_2, firstdate=firstdate,start_date_dt=dates[i])
        ...
        firm_MES.to_csv(os.path.join(path,r'HK_2Step_Adaboost_singlefirm_%d.csv'%firmcode))
        gc.collect()
...
for firmcode in firmcodes:
    try:
        mes = cal_MSE_firm(firmcode)
        mes.to_csv(os.path.join(path,r'HK_2Step_Adaboost_singlefirm_%d.csv'%firmcode))
        all_mes.append(mes)
        firm_MES_all = pd.concat(all_mes)
        firm_MES_all.to_csv('HK_2Step_MES_Adaboost.csv')
    except:
        pass
