...
#           're_excess_2','equ_vol_2','Rsysh','Varh']].iloc[xydata.shape[0]-1:xydata.shape[0],:]
    ...
    weights_0 = tf.Variable(tf.random_normal([n_inputs,num_layers_0], 
                                             stddev=(1/tf.sqrt(float(n_inputs)))))
    bias_0 = tf.Variable(tf.random_normal([num_layers_0], stddev=(1/tf.sqrt(float(n_inputs)))))

    

    weights_1 = tf.Variable(tf.random_normal([num_layers_0,num_layers_1], 
                                             stddev=(1/tf.sqrt(float(num_layers_0)))))
    bias_1 = tf.Variable(tf.random_normal([num_layers_1], stddev=(1/tf.sqrt(float(num_layers_0)))))

    weights_2 = tf.Variable(tf.random_normal([num_layers_1,n_outputs], 
                                             stddev=(1/tf.sqrt(float(num_layers_1)))))
    bias_2 = tf.Variable(tf.random_normal([n_outputs]))

    # Activation function and dropout
    hidden_output_0 = tf.nn.relu(tf.matmul(X,weights_0)+bias_0)
    hidden_output_0_dropout = tf.nn.dropout(hidden_output_0, keep_prob=keep_prob)
    
    hidden_output_1 = tf.nn.relu(tf.matmul(hidden_output_0_dropout,weights_1)+bias_1)
    hidden_output_1_dropout = tf.nn.dropout(hidden_output_1, keep_prob)
    
    out = tf.matmul(hidden_output_1_dropout,weights_2)+bias_2
...
#    dates = list(date_df.index) 
    ...
    for i in range(len(dates)):
        ...
        firm_MES.to_csv(os.path.join(path,r'US_2Step_ANN2_singlefirm_%d.csv'%firmcode))
        gc.collect()
...
for firmcode in firmcodes:
    try:
        mes = cal_MSE_firm(firmcode)
        mes.to_csv(os.path.join(path,r'US_2Step_ANN2_singlefirm_%d.csv'%firmcode))
        all_mes.append(mes)
        firm_MES_all = pd.concat(all_mes)
        firm_MES_all.to_csv('US_2Step_ANN2.csv')
    except:
        pass
